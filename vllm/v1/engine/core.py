...
class EngineCore:
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        if not self.scheduler.has_requests():
            return {}, False
        print(f"[STEP 7 - REQ_FLOW_10] EngineCore.step() - calling scheduler.schedule()")
        scheduler_output = self.scheduler.schedule()
        print(f"[STEP 7 - REQ_FLOW_11] EngineCore.step() - executing model | total_tokens={scheduler_output.total_num_scheduled_tokens}")
...
class EngineCoreProc(EngineCore):
    ...
    def _handle_client_request(
        ...
        if request_type == EngineCoreRequestType.ADD:
            ...
            print(f"[STEP 6 - REQ_FLOW_06] EngineCore dispatching ADD to scheduler | request_id={req.request_id}")
            ...
            while True:
                for input_socket, _ in poller.poll():
                    ...
                    if request_type == EngineCoreRequestType.ADD:
                        ...
                        print(f"[STEP 5 - REQ_FLOW_05] EngineCore received ADD request via ZMQ | request_id={req.request_id}")
                        try:
