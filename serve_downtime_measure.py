...
Usage:
  ...
  python3 probe.py probe --url http://localhost:8006/health --interval 50 --log scale_up.log

  # Analyze mode - compute downtime from a log file
  python3 probe.py analyze --log scale_up.log

  # Compare two scenarios
  python3 probe.py compare --log1 scale_up.log --log2 restart.log

=============================================================================
IMPORTANT: To avoid the probe being killed by cleanup scripts, copy this
script to /tmp and rename it to avoid "vllm" and "serve" in the path/name:

  cp serve_downtime_measure.py /tmp/probe.py
  cd /tmp
  python3 probe.py probe --url http://localhost:8006/health --interval 50 --log restart.log --verbose

This avoids being killed by:
  - pkill -f vllm
...
TERMINAL 1: vLLM Serve
  # Start the serve
  experimental/serve.sh --dp-size 4

TERMINAL 2: Probe Script (run from /tmp to avoid being killed)
  cp /workspace/vllm/serve_downtime_measure.py /tmp/probe.py
  cd /tmp

  # Scenario 1: Scale-up measurement
  python3 probe.py probe --url http://localhost:8006/health --interval 50 --log scale_up.log
  # Trigger scale-up from Terminal 3, wait for SUCCESS, then Ctrl+C
  python3 probe.py analyze --log scale_up.log

  # Scenario 2: Restart measurement
  python3 probe.py probe --url http://localhost:8006/health --interval 50 --log restart.log
  # Kill and restart serve from Terminal 3, wait for SUCCESS, then Ctrl+C
  python3 probe.py analyze --log restart.log

  # Compare both scenarios
  python3 probe.py compare --log1 scale_up.log --log2 restart.log

TERMINAL 3: Trigger Operations
  # For scale-up:
  curl -X POST http://localhost:8006/scale_elastic_ep -d '{"new_ep_size": 8}'

  # For restart:
  pkill -9 vllm && ./cleanup_all.sh && experimental/serve.sh --dp-size 4

Timeline:
---------
  Tab 2: Start probe ────> [keep running] ──────────────────> Ctrl+C (stop)
  Tab 3:                   Kill/Scale ──> Restart ──> Server up
                                │                        │
                                └── Failures start ──────┘ Successes resume
                                    (capture this window!)
"""
...
Examples:
  ...
  %(prog)s probe --url http://localhost:8006/health --interval 50 --log scale_up.log
