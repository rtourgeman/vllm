...
echo "[${my_node}] waiting for vLLM to start (watching ${VLLM_LOG})"
waited=0
while ! grep -q "Application startup complete" "${VLLM_LOG}" 2>/dev/null; do
    if ! kill -0 "${vllm_pid}" >/dev/null 2>&1; then
        ...
        echo "[${my_node}] last 20 lines of server log:" >&2
        tail -20 "${VLLM_LOG}" >&2 || true
        wait "${vllm_pid}" || true
        exit 1
    fi
    if (( waited >= SERVER_WAIT_TIMEOUT )); then
        echo "[${my_node}] timed out waiting for vLLM startup" >&2
        echo "[${my_node}] last 20 lines of server log:" >&2
        tail -20 "${VLLM_LOG}" >&2 || true
        exit 1
    fi
